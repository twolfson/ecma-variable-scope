...
var WITH_SCOPE_TYPE = 'with';
var BLOCK_SCOPE_TYPE = 'block';
var LEXICAL_SCOPE_INFO_TYPE = 'lexical';
var BLOCK_SCOPE_INFO_TYPE = 'block';
...
function resolveScope(baseNode, identifiers, scopeType) {
  ...
  if (identifiers.length) {
    ...
    var scopeTypes = scopeType === LEXICAL_SCOPE_INFO_TYPE ? [LEXICAL_SCOPE_INFO_TYPE] : [LEXICAL_SCOPE_INFO_TYPE, BLOCK_SCOPE_INFO_TYPE];
    while (scope) {
    ...
    enter: function (node, parent) {
      ...
      switch(node.type) {
        ...
        case 'VariableDeclaration':
          ...
          scopeType = node.kind === 'let' ? BLOCK_SCOPE_INFO_TYPE : LEXICAL_SCOPE_INFO_TYPE;
        ...
        case 'ArrowFunctionExpression':
          ...
          if (node.id) {
            resolveScope(node, [node.id], LEXICAL_SCOPE_INFO_TYPE);
          }

          // Resolve scope for the fn parameters (will resolve to fn)
          if (node.params[0]) {
            resolveScope(node.params[0], node.params, LEXICAL_SCOPE_INFO_TYPE);
          }
          break;
        ...
        // `try { /* ... */ } catch (name) { /* ... */ }
        case 'CatchClause':
          resolveScope(node.param, [node.param], BLOCK_SCOPE_INFO_TYPE);
          break;
        // `var a = [1 for (value of ['hello'])];`
        case 'ComprehensionBlock':
          resolveScope(node.left, [node.left], BLOCK_SCOPE_INFO_TYPE);
          break;
      }
    }
  });
...
ecmaVariableScope.LEXICAL_SCOPE_TYPE = LEXICAL_SCOPE_TYPE;
ecmaVariableScope.WITH_SCOPE_TYPE = WITH_SCOPE_TYPE;
ecmaVariableScope.BLOCK_SCOPE_TYPE = BLOCK_SCOPE_TYPE;
ecmaVariableScope.LEXICAL_SCOPE_INFO_TYPE = LEXICAL_SCOPE_INFO_TYPE;
ecmaVariableScope.BLOCK_SCOPE_INFO_TYPE = BLOCK_SCOPE_INFO_TYPE;

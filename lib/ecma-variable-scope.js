...
// Define constants
var LEXICAL_SCOPE_BOUNDARIES = ['lexical'];
var BLOCK_SCOPE_BOUNDARIES = ['lexical', 'block'];
...
function resolveScope(baseNode, identifiers, scopeTypes) {
  ...
  if (identifiers.length) {
    ...
    // TODO: Add property to each identifier to mark it as `block`/`lexical`
    ...
    enter: function (node, parent) {
      ...
      switch(node.type) {
        ...
        case 'VariableDeclaration':
          ...
          scopeType = node.kind === 'let' ? BLOCK_SCOPE_BOUNDARIES : LEXICAL_SCOPE_BOUNDARIES;
        ...
        case 'ArrowFunctionExpression':
          ...
          if (node.id) {
            resolveScope(node, [node.id], LEXICAL_SCOPE_BOUNDARIES);
          }

          // Resolve scope for the fn parameters (will resolve to fn)
          if (node.params[0]) {
            resolveScope(node.params[0], node.params, LEXICAL_SCOPE_BOUNDARIES);
          }
          break;

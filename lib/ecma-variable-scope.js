...
function resolveScope(node, identifiers, scopeType) {
  ...
  if (identifiers.length) {
    ...
    while (scope) {
      ...
      console.log(node, scope, scope.type, scopeType);
...
function ecmaVariableScope(ast) {
  ...
  estraverse.traverse(ast, {
    ...
    enter: function (node, parent) {
      ...
      // Otherwise, save `nearestScope` in a reasonable fashion
      } else {
        node.nearestScope = scope;
      }
    ...
    enter: function (node, parent) {
      ...
      switch(node.type) {
        ...
        case 'FunctionDeclaration':
          ...
          // TODO: Scope is wrong for this one
          ...
          break;
        // TODO: Cover `FunctionExpression` -> `[].map(function name (params) {})`

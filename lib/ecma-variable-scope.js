...
function verifyIdentifier(node, parent) {
  ...
  estraverse.traverse(ast, {
    ...
    enter: function (node) {
      ...
      if (node.type === 'Identifier' && params.enter) {
        // If the node's parent is a `MemberExpression`
        var parent = parents[parents.length - 1];
        if (parent && parent.type === 'MemberExpression') {
          // If the node expression is computed (e.g. `key` in `obj[key]` vs `log` in `console.log`)
      ...
      // Save the node as the next one's parent
      parents.push(node);
    },
    exit: function (node) {
      // Pop the node as a parent
      parents.pop();

      // DEV: Commented for code coverage
      // // If the node is an identifier, run our `exit` on it
      // if (node.type === 'Identifier' && params.exit) {
      //   params.exit(node);
      // }
    }
  });
